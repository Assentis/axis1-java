...
public class Rdn implements Serializable, Comparable {
    ...
    public Object getValue() {
        return ((Rdn) entries.get(0)).getValue() ;
    }
     ...
     */
    public String getType() {return ((Rdn) entries.get(0)).getType();
    }
    ...
    public int compareTo(Object obj) {
        ...
        for (int i = 0; i < minSize; i++) {
            ...
            int diff = ((Rdn) entries.get(i)).compareTo(that.entries.get(i));
            if (diff != 0) {
                return diff;
            }
        }
    ...
    private static class RdnEntry implements Comparable {
        ...
        public int compareTo(Object that) {
            int diff = type.compareToIgnoreCase(((RdnEntry) that).type);
            if (diff != 0) {
                return diff;
            }
            if (value.equals(((RdnEntry) that).value)) {     // try shortcut
                return 0;
            }
            return getValueComparable().compareTo( ((RdnEntry)       that).getValueComparable());
        }
